...
$tot_itens = 0;
               ...
               $desconto = str_replace($source, $replace, $desconto_1);
                ...
                $q = $pdo->prepare($sql);
                $q->execute(array($idPedidoRecente));
                Banco::desconectar();
...
$txt_rodape[] = ' '; // força pular uma linha
    ...
    if($_POST){
            ...
            $pdo = Banco::conectar();
            $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $sql = "UPDATE ordem_pedido_balcao  set  atendente= ?, troco=?, hora =?, valor_pedido=?,  valor_itens=?,  desconto=? , cliente=?  WHERE id_pedido = ?";
            $q = $pdo->prepare($sql);
            $q->execute(array($_POST['nome_atendente'], $_POST['troco'], $hora,  $total,  $tot_itens, $desconto, $_POST['nome_cliente'], $idPedidoRecente));
            Banco::desconectar();
