      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <div class="card shadow mb-4">
              <button type="button" class="btn btn-success" onclick="location.href='adicionar_cliente.php'" style="height:30px; font-size: 13px; width:100px; margin: 15px;">Adicionar <i class="fas fa-plus"></i></button>
            <div class="card-header py-3">
                
              <h6 class="m-0 font-weight-bold text-primary">Informações sobre os clientes cadastrados</h6>
            </div>
       ...
       $q = $pdo->prepare($sql);
       $q->execute(array($row['bairro']));
       $valor = $q->fetch(PDO::FETCH_ASSOC);
       Banco::desconectar(); 
                            
                            ?>
                            <?=number_format($valor['valorEntrega'],2,",","")?></td>
                      
                      <th  style="width: 210px;">
                      <a  href="CRUD/updateCliente.php?id=<?=$row['id']?>"  class="btn btn-primary" style="height:30px; font-size: 13px; margin: 1px; width: 80px; float: left; ">Editar <i class="far fa-edit"></i></a>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
       Deletar Esse Cliente do Sistema?

      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancelar</button>
        <a href="CRUD/deleteCliente.php?id=<?=$row['id']?>"  class="btn btn-danger">Confirmar</a>
      </div>
    </div>
  </div>
</div>
